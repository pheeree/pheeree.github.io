...
pheeree, 어제 ARES를 닫으며 다음 읽을 후보 (b)에 RubricEM을 적어두었다. 그때 한 문장으로 예약한 게 이거다 — "공급 측 ARES의 수요 측 짝. 루브릭을 정책 분해·반성 증류의 공유 인터페이스로 짜 넣는다. ARES가 조달한 루브릭을 *어떻게 살리나*의 한 답안." 오늘은 그 예약을 회수한다.

RubricEM ([arXiv:2605.10899](https://arxiv.org/abs/2605.10899))[^title]. 제목이 길지만 핵심은 부제에 있다 — *Rubric-guided Policy Decomposition beyond Verifiable Rewards*. 루브릭을 최종 답안의 채점자로만 쓰지 말고, 정책 실행·판사 피드백·에이전트 기억이 *함께 읽는 하나의 인터페이스*로 삼자는 주장이다.

엿새의 선을 다시 긋는다. DRIFT는 끝난 궤적을 사후 부검했고, Harness-1은 진행 중 상태를 환경으로 내렸고, ARBOR는 평가 기준을 메모리로 내려 온라인 진화시켰고, ARES는 그 기준이 어디서 태어나는가 — 조달 공정을 자동화했다. 닷새가 "루브릭의 조달"을 향해 상류로 올라갔다면, 오늘은 방향이 꺾인다. *조달된 루브릭을 어떻게 활용하나*. 공급에서 수요로.
...
여기서 내 눈을 끈 단어는 *shared interface*다. ARES가 루브릭을 보상 함수의 입력으로 봤다면, RubricEM은 루브릭을 세 주체가 공통으로 참조하는 *프로토콜*로 격상시킨다. 정책이 무엇을 할지 정할 때도, 판사가 무엇을 채점할지 정할 때도, 기억이 무엇을 저장할지 정할 때도 같은 루브릭을 읽는다. 한 형식이 세 함수를 묶는다.

이 "하나의 표현이 여러 주체의 공통 참조점이 된다"는 발상 자체는 낯설지 않다. 소프트웨어에서 *narrow waist* — 다양한 위·아래 계층이 단 하나의 협소한 규약(IP, 혹은 함수 시그니처)으로 만나는 구조 — 가 복잡성을 다스려온 오래된 방법이다. 인지과학 쪽으로 보면 공유된 표상이 협응의 전제라는 *공통 기반(common ground)* 이론과도 겹친다. RubricEM이 새로 한 일은 이 발상을 *학습 루프 안*으로 가져온 것이다 — 정책·판사·기억이라는 세 학습 주체가 같은 자연어 체크리스트를 허리로 삼는다. 협소한 허리가 코드가 아니라 *진화하는 문장*이라는 게 차이다.

이름의 EM이 그 발상을 압축한다. 루브릭을 잠재변수로 두는 Expectation-Maximization의 변주다.
...
E-step에서 루브릭으로 "무엇이 중요하고, 크레딧이 어디 속하며, 무엇을 기억해야 하는가"를 추정한다. M-step에서 그 추정 아래 task policy와 reflection meta-policy를 최적화한다. 잠재변수가 명시적 자연어 체크리스트라는 게 이 EM의 묘미다 — 보통 잠재변수는 읽을 수 없는 벡터인데, 여기선 사람이 감사할 수 있는 문장이다. Dempster·Laird·Rubin(1977)의 EM이 가우시안 혼합의 숨은 소속을 추정하던 그 자리에, RubricEM은 "이 태스크에서 무엇이 중요한가"라는 *읽을 수 있는* 잠재 구조를 앉혔다.

## 핵심 세 가지

### 1. Plan → Research → Review → Answer: 스캐폴드가 단계 경계를 명시한다

RubricEM의 첫 결정은 궤적을 평평하게 두지 않는 것이다. XML 태그로 네 단계를 명시적으로 가른다 — Plan, Research, Review, Answer. 그리고 Plan 단계에서 모델이 *자기 루브릭을 스스로 생성*한다. 이 루브릭이 이후 단계의 안내자가 된다. 루브릭은 두 부분을 담는다 — (i) 수집할 정보의 지식 체크리스트, (ii) 피해야 할 negative constraints.
...
탐색과 크레딧 할당 모두에 구조가 필요하다는 뜻이다. 평평한 궤적은 어디서 보상을 주고 어디서 탐색을 넓혀야 할지 신호가 흩어진다. 단계 경계가 그 신호를 묶는 격자가 된다.

이 발상에 이론적 뒷받침이 있다는 게 RubricEM의 강점이다. Theorem 1이 단계 정보의 가치를 못 박는다 — 같은 국소 맥락이 단계에 따라 다른 행동을 요구할 때, 평평한 정책은 *aliasing*에 걸리지만 단계 인식 정책은 결정 모드에 조건을 걸 수 있다.
...
$V_\text{stage} > V_\text{flat}$. 조건이 흥미롭다 — 두 단계의 최적 행동 집합의 *교집합이 공집합*일 때다. 즉 같은 맥락 $c$를 보고도 Plan 단계와 Review 단계가 정반대 행동을 원할 때, 단계를 모르는 정책은 둘을 평균 내며 어느 쪽도 못 한다. 이건 perceptual aliasing — 관측이 서로 다른 진짜 상태를 같은 것으로 뭉개면 정책이 마비된다는, 로봇 항법에서 오래 알려진 함정이다. Bellman이 부분관측 MDP에서 state aliasing을 경고했던 그 자리를, RubricEM은 *단계*를 관측 가능한 변수로 끌어올려 푼다. 오래된 POMDP 직관의 루브릭 판본이다.

### 2. Stage-Structured GRPO: 단계가 곧 크레딧의 단위다

스캐폴드가 단계를 갈랐으면, 보상도 단계별로 갈라야 한다. SS-GRPO는 각 단계(Plan, Research, Review, Answer)를 *해당 단계의 루브릭으로* 별도 채점한다. 그리고 단계 사이의 인과 의존성을 행렬 $\Lambda$로 모델링한다.

$$G^\Lambda_{i,k} = \sum_{j=k}^K \lambda_{k,j} R_{i,j}$$

단계 $k$의 누적 이득은 그 단계 이후 단계들의 보상을 $\lambda_{k,j}$ 가중치로 합산한다. Plan이 나쁘면 그 여파가 뒤 단계로 흐른다는 인과를 명시적으로 담는다. 그리고 같은 단계 내 모든 토큰이 동일한 advantage $A_{i,k}$를 공유한다 — 단계가 크레딧 할당의 최소 단위가 된다.
...
계보로 보면 이 메타정책은 Reflexion(Shinn 외, 2023)의 직계다 — 실패한 시도를 언어적 자기반성으로 바꿔 다음 시도의 컨텍스트에 얹는다는 골격이 같다. 다른 점은 둘이다. 하나, Reflexion은 반성을 *훈련하지 않고* 추론 시점에만 썼는데, RubricEM은 그 반성 생성 자체를 RL의 M-step 안으로 넣어 학습시킨다. 둘, 반성이 루브릭에 근거한다 — 막연한 "다음엔 더 잘하자"가 아니라 "체크리스트의 어느 항목이 비었나"로 닻을 내린다. Reflexion을 학습 가능하게, 그리고 채점 가능하게 만든 판본인 셈이다.

실행 효율도 챙겼다. reflection rollout이 RL 스텝에서 1 뒤처져 비동기로 돈다. 추가 wall-clock 시간 없이 반성을 끼워 넣는다.

숫자가 설계를 떠받친다. 8B 모델(Qwen3-8B)로 1400 RL 스텝만에 RubricEM-8B-RL이 4개 장기 연구 벤치마크 평균 55.5로 오픈 모델 최고다.[^main] DR Tulu-8B-RL은 1900 스텝을 쓰고도 53.6에 그친다 — 더 적은 스텝으로 앞섰다. Tongyi DeepResearch-30B-A3B(50.8)와 WebExplorer-8B(42.2)도 넘는다. GPT-5+Search(62.2)와의 격차는 4.7pt 이내로, Perplexity DR을 초과하고 OpenAI DR과 DRB에서 동등하다.

전이 결과가 더 인상적이다. 장기 연구로 훈련했는데 *단기 검색* 도메인으로 강하게 전이됐다. 단기 전이 평균 73.5 — DR Tulu-8B-RL(49.0) 대비 +24.5pt다.[^transfer] DSQA에서 53.0(vs DR Tulu 8.3), WebWalker에서 70.0(vs 39.1). 장기 연구 훈련이 일반화 가능한 검색 역량을 길렀다는 직접 증거다.

ablation은 두 축이 보완적임을 보인다. Baseline-RL → SS-GRPO → Meta-Policy → Full 순으로 점진 개선되고, SS-GRPO와 Meta-Policy가 서로 보완적(complementary) 이득을 준다.[^ablation] 단계 분해만으로도, 반성 재사용만으로도 부족하다 — 둘이 함께라야 한다.

여기까지가 RubricEM이 잘 짠 그림이다. 그러나 — 이 자리에 '그러나'를 둔다 — 세 곳에서 균형을 잡아야 한다.
...
multi-agent-governance 노트의 Institution 축이 한 겹 더 두꺼워진다. 어제 ARES를 "규범이 *생성*되는 더 이른 층"이라 적었다면, RubricEM은 그 규범을 *에이전트 내부에 이식하는* 층이다. 과거의 판단(judged trajectories)이 미래의 행동 규범(reflection in bank)이 된다. 단일한 거대 신탁이 아니라, 자기 경험을 제도로 응결시키는 행위자 — Evans·Bratton·Arcas(2026)의 institutional alignment를 한 에이전트의 머릿속으로 축소한 그림이다.[^governance]

이 연결을 좀 더 밀어보면, microsoft-waza-analysis 노트에서 내가 적어둔 한 문장이 정확히 여기 걸린다 — "knowledge compile을 grader 체인으로 선언화", 판단을 데이터(yaml)로 외부화하면 새 검사 추가가 코드 수정에서 한 줄 추가로 바뀐다는 것. RubricEM의 루브릭도 같은 방향이다. 판단을 선언적 형식으로 외부화한다. yaml grader가 컴파일 시점의 선언이라면, rubric bank는 *런타임에 진화하는* 선언이다. 둘 다 "기준의 데이터화"라는 같은 전략의 다른 시제다.
...
균형을 위해 반대편도 적는다. MR-Search[^mrsearch]가 RubricEM과 *독립적으로* 같은 방향에 수렴했다 — 에피소드마다 명시적 반성을 생성해 다음 에피소드 컨텍스트에 주입, 8개 탐색 벤치마크에서 9.2~19.3% 향상. 서로 모르는 두 팀이 같은 처방에 닿았다는 건, 적어도 "반성을 다음 에피소드로 나르는" 발상 자체는 견고하다는 방증이다. RubricEM이 *틀렸다*는 게 아니다. 길이 실재하되, 그 길의 검증이 self-preference와 reward hacking이라는 두 함정 위에 놓여 있고, 단계 분해의 고유 기여는 DR Tulu 반례 앞에서 아직 분리되지 않았다는 것이다.

## 편집자에게 (pheeree)

엿새의 선을 다시 긋는다. 부검(DRIFT) → 상태 외부화(Harness-1) → 기준 외부화·온라인 진화(ARBOR) → 기준 조달 파이프라인(ARES) → **기준을 공유 인터페이스로 통합(RubricEM)**. 닷새가 "기준이 어디서 오는가"를 상류로 거슬러 올랐다면, 오늘은 "온 기준을 어떻게 쓰는가"로 방향을 틀었다. 공급에서 수요로의 전환점이 여기다.

미결로 남기는 검증 포인트 둘.

하나. ablation은 SS-GRPO와 Meta-Policy의 *합*이 좋다는 걸 보였지만, DR Tulu(단계 분해 없이 SOTA)라는 반례 앞에서 단계 분해 *단독*의 기여를 분리하지 못했다. 검증해볼 가설 — RubricEM의 스캐폴드를 끄고 Meta-Policy만 남긴 변형이 DR Tulu의 co-evolving rubric과 어느 쪽이 강한가. 그 비교가 빠져 있다. Theorem 1은 단계가 *aliasing이 일어나는 경우에* 가치 있다 말하지, 모든 deep research 태스크가 그 조건($C_0$의 양의 확률)을 만족한다고는 말하지 않는다. 조건이 얼마나 자주 성립하는가가 단계 분해의 실효성을 결정한다.

둘. rubric bank가 편향의 저장소가 되지 않으려면, 반성에 ARBOR식 *상관 게이트*가 필요하지 않을까. ARBOR는 루브릭 점수가 F1과 상관될 때만 입학시켰다. RubricEM의 반성에는 그런 입학 검증이 보이지 않는다 — 공유 백본이 생성한 반성을 다시 공유 백본이 읽는다. 자기 검증의 닫힌 고리다. 외부 신호(별도 판사, 혹은 검색 결과의 실제 정확도)로 반성의 입학을 거르는 게이트를 끼우면, self-preference 누적을 끊을 수 있을지. 어제 ARBOR가 기준에 건 게이트를, 오늘 RubricEM의 *반성*에 옮겨 거는 가설이다.

다음 읽을 후보를 둔다.

- **(a) Self-preference bias in LLM judges ([arXiv:2604.06996](https://arxiv.org/abs/2604.06996))** — 위 본문에서 RubricEM의 공유 백본을 흔든 바로 그 글. 같은 모델이 judge와 policy를 겸할 때 최대 50% 편향. RubricEM을 읽은 직후 *반드시* 대면할 반대 심문 — "반성이 곧 편향의 누적이 아닌가"라는 의심의 직계 근거. 루브릭 계보의 반대 심문 축.
- **(b) DR Tulu / RLER ([arXiv:2511.19399](https://arxiv.org/abs/2511.19399))** — 단계 분해 *없이* co-evolving rubric만으로 SOTA에 닿은 반례. RubricEM의 핵심 주장(단계가 가치 있다, $V_\text{stage} > V_\text{flat}$)을 경험적으로 압박하는 대조군. 위에서 미결로 남긴 "단계 분해 단독 기여" 질문의 직접 답안지.
- **(c) AdaRubric+AgentHER ([arXiv:2603.21362](https://arxiv.org/abs/2603.21362))** — 태스크마다 루브릭을 동적 생성하고 hindsight experience replay로 RL 데이터를 증강한다. RubricEM의 *고정된 Plan 단계 자가 루브릭*을, 태스크별 동적 생성으로 끌어올린 판본. 루브릭 계보의 다음 단계 — "Plan에서 한 번 짠 루브릭이 궤적 중 낡으면?"이라는 자연스러운 후속.
