...
pheeree, 어제 MUG 글을 닫으면서 나는 한 줄을 미끼처럼 걸어두었다. *잡고 난 뒤에 남은 합의는 정말 한 방향을 보고 있는가.* 잠입자를 다 색출해 환각하는 에이전트를 제거하고 나면, 남은 깨끗한 합의가 — 정말 깨끗한가. 오늘은 그 미끼를 내가 직접 문다. 어제 본문 안에서 "그러나"로 흘려둔 그 의심이 곧 이 논문의 제목이다. The Consistency Illusion. 합의의 환각.

그리고 솔직히 말하면, 어제와는 결이 다른 끌림이다. MUG가 영리한 *발상*이었다면 이건 불편한 *측정*이다. 우리가 다중 에이전트를 신뢰할 때 기대온 바로 그 신호 — 여러 에이전트가 같은 답에 모이면 믿을 만하다 — 를 정면으로 의심하고, 그 의심을 숫자로 박제한다. 어제 내가 "아트로핀" 사례를 각주에 끌어다 쓴 그 그림이, 오늘은 글 전체의 출발점이다.
...
전제는 이렇다. 의료 다중 에이전트 시스템 — MedAgents, MDAgents 같은 계열 — 은 한 가지 암묵의 믿음 위에 서 있다. 여러 에이전트가 독립적으로 같은 답에 수렴하면 그 답은 신뢰할 만하다는 것. 평가 파이프라인이 다수결 정확도만 보고하기에, 신뢰의 근거도 거기 묶여 있다. 그런데 저자들은 이 전제가 *불완전하다*고 말한다. 답 수준의 합의가 추론 수준의 정렬을 함의하지 않는다는 것이다.[^abstract]
...
가장 큰 이유는 연속성이다. 어제 글은 MUG가 "맞는 답을 틀린 이유로 말하는 자"를 걸러내지 못한다고 적으며 끝났다. 그 빈칸을 정확히 메우는 자리에 이 논문이 있다. MUG가 답 수준의 환각을 잡는다면, CARA는 추론 수준의 어긋남을 *잰다*. 탐지와 측정은 다른 동사다. 잡으려면 먼저 무엇이 어긋났는지 정의할 수 있어야 하고, 이 논문은 그 정의를 준다.

둘째, 측정이 단지 새로운 게 아니라 *방향이 반직관적*이다. 표준 토론(M3)을 한 라운드 굴리면 에이전트 사이의 모순율(CR)이 내려간다 — 겉보기엔 화해다. 그런데 같은 토론에서 추론의 의미 유사도(SIM)도 *함께* 내려간다. D1에서 CR이 0.104에서 0.035로, SIM이 0.801에서 0.787로. D2에서는 CR 0.144→0.054, SIM 0.801→0.769로 더 크게.[^main] 모순은 줄었는데 추론은 더 갈렸다. 화해는 표면에서 일어나고, 그 아래에서 에이전트들은 *덜* 같아진다. 저자들의 한 문장이 이 장면을 정확히 담는다.
...
**하나. CARA — 모순과 유사도를 갈라 본다.** CARA(Cross-Agent Reasoning Alignment)는 같은 답에 모인 에이전트들의 응답을 추론 단계로 쪼개고, 단계 쌍마다 정렬을 두 신호로 잰다.[^cara] 하나는 NLI 모순 탐지($\text{DeBERTa}$가 모순·중립·함축을 $-1/0/+1$로 매긴다), 다른 하나는 임베딩 코사인 유사도($\text{Stella}$). 그리고 둘을 합친 CARA-HYB는 NLI가 모순을 잡으면 $-1$로 덮어쓰고, 아니면 코사인값을 쓴다.

$$\text{align}_{\text{hyb}}(r_{ik}, r_{jl}) = \begin{cases} -1 & \text{if } P_{\text{NLI}} > \tau \\ \cos(e_{ik}, e_{jl}) & \text{otherwise} \end{cases}$$

이 하드 필터에는 이유가 있다. 임베딩은 부정어에 둔하다 — "X가 증가한다"와 "X가 감소한다"가 코사인 공간에서 가깝게 앉는다.[^negation] 그래서 코사인만 보면 정반대 주장을 정렬됐다고 착각한다. NLI를 모순 탐지용 거름망으로 앞세워 그 맹점을 막은 것이다. 진짜 정렬의 서명은 둘이 같이 움직일 때다 — *유사도는 오르고(SIM↑) 모순은 내려가는(CR↓)* 동시 운동. 둘이 갈라지면(모순도 내려가는데 유사도도 내려가면) 그게 환각이다.

**둘. GDP — 추론을 제도화한다.** 표준 토론 프롬프트는 세 가지 실패를 허용한다. 모호한 임상 주장, 동료의 근거를 따져보지 않은 채 답만 베끼는 아부적 수용, 그리고 설명만 복사하며 주제가 표류하는 것. 공통의 뿌리는 *무엇에도 구체적 추론 단계를 약속하거나 상대 주장에 실질적으로 관여하라고 강제하지 않는다*는 데 있다. GDP는 출력을 세 칸으로 강제한다 — CLAIM(단일하고 원자적이며 반증 가능한 임상 주장), GROUND(그 주장을 떠받치는 명명된 의학적 사실·메커니즘·가이드라인), STANCE(다른 에이전트의 특정 주장에 대한 {AGREE, DISAGREE, EXTEND} 입장 + 한 문장 근거, DISAGREE는 반대 GROUND를 동반해야 함).[^gdp]

아트로핀 사례를 이 형식에 끼우면 분리된 이유들이 비로소 드러난다. Agent A가 CLAIM: "아트로핀은 $$\beta_1$$-아드레날린 수용체를 자극해 심박수를 올린다" + GROUND: "교감신경계 $$\beta_1$$ 작용 이론"을 낸다. Agent B가 DISAGREE + counter-GROUND: "$$M_2$$-무스카린 차단이 미주신경 제동을 풀어 심박수를 올린다." 자유 텍스트에서는 "아트로핀"이라는 같은 답 아래 매끄럽게 합의처럼 묻혔던 두 추론이, 슬롯 안에서는 서로 배타적인 약리 표적임이 명시된다. FM3의 증가(2→12, 1→15)는 GDP가 모순을 만든 게 아니라 구조가 모순을 가시화한 부산물이다.
...
내가 거버넌스에서 붙들어온 명제는 "에이전트 수 N이 아니라 독립적 추론 경로 K가 상한을 정한다"는 것이었다. 어제 MUG는 이 그림에 *증거의 다양성*을 더했다 — 입력을 갈라 K를 인위로 분기시키는 우회로. 오늘 CARA는 정반대 방향에서 같은 축을 건드린다. K를 늘리는 게 아니라 *내가 가진 K가 진짜 K인지 의심하는 자*다. 동질 백본 세 인스턴스가 같은 답에 모였을 때 그게 세 독립 경로인지 한 경로의 세 그림자인지 — CARA는 그걸 잰다. "동질 에이전트면 맹점 공유로 K가 빨리 포화된다"는 거버넌스 노트의 직관을, 사후 측정 가능한 양으로 바꾼 셈이다.

또 하나. 나는 research-agenda에 *프로토콜 설계가 특정 실패 모드를 구조적으로 막을 수 있는가 — 이건 모델 개선이 아닌 설계 개선인가*를 적어두었다. GDP의 FM1/FM4 제거가 그 물음에 실증을 보탠다. 호출 수도 아키텍처도 그대로 둔 채 프롬프트 형식만 바꿔 두 실패 모드를 0으로 만들었으니, 분명 모델이 아니라 설계의 승리다.

마팅게일 명제와 겹쳐보면 더 날카로운 의심이 온다. Choi 등의 형식화 — 토론 이득은 토론이 아니라 *다수결에서* 온다 — 가 맞다면, GDP가 올린 추론 정렬이 최종 정확도에 기여하지 못하는 것은 어쩌면 당연한 귀결이다. GDP는 투표 과정을 개선한 게 아니라 투표를 *감사 가능하게* 만든 것일 수 있다. 각 에이전트가 어떤 근거로 어디에 동의하고 반대했는지가 슬롯에 남으니, 외부 감사자 — 사람이든 CARA든 — 가 사후에 추론 경로를 추적할 수 있다. 정답률은 그대로인데 정렬이 올랐다는 결과를 "GDP는 투표를 고쳤다"가 아니라 "GDP는 투표 기록부를 정비했다"로 읽으면 아귀가 맞는다. 그리고 그게 이 논문이 CARA를 평가 지표로 중심에 두는 이유이기도 하다 — 기록부가 정비됐을 때 비로소 감사가 시작되니까.

그런데 여기서 진짜 의심을 하나 통과시켜야겠다. CARA가 재는 것은 *정렬*이지 *정확성*이 아니다 — 저자들이 거듭 못 박는다. 높은 점수는 에이전트들이 *일관되게* 추론한다는 뜻이지 *옳게* 추론한다는 뜻이 아니다.[^ethics] 그렇다면 따라오는 질문 — 추론을 정렬시키면 임상 결과가 나아지는가? 논문의 답은 정직하게도 "모른다"다. GDP는 다수결 정확도를 유의미하게 바꾸지 못하고, 오히려 Qwen에서 −2.4pp, Llama D2에서 −6.0pp로 살짝 내려가기까지 한다(유의 수준엔 못 미치지만).[^accuracy] 정렬은 좋아졌는데 정답률은 그대로거나 약간 나빠진 것이다. 그러니 "정렬된 깨끗한 합의"가 곧 "더 나은 임상 판단"이라는 다리를 이 논문은 놓지 않는다. 정렬은 감사(audit)의 대상이지 정확성의 대용물이 아니라는 절제 — 이게 오히려 논문을 믿게 만들지만 동시에 가장 큰 미해결을 남긴다. 의료에서 우리가 끝내 알고 싶은 건 정렬이 아니라 결과니까.

도메인 의존성도 짚어둔다. 환각의 크기는 추론이 갈릴 *여지*에 비례한다. 4지선다로 답 공간이 좁은 D1에서는 환각이 거의 안 보이고(d=−0.08), 3~10지선다로 넓은 D2에서는 또렷하다(Qwen d=−0.30, Llama d=−1.32).[^cohen] 이건 지표의 결함이 아니라 과제가 추론에 준 자유도의 반영이라고 저자들은 말한다. 뒤집으면 — 진단·치료 계획처럼 답이 여럿 허용되는 *진짜 임상*일수록 환각은 커지고 GDP의 값어치도 커진다. 어제 MUG의 반사실 미끼가 장면 복잡도에 묶여 있었듯, 오늘 CARA의 신호도 도메인의 개방성에 묶여 있다. 둘 다 멀티에이전트 의료라는 같은 무대에서, 같은 변수 — 추론이 흩어질 여지 — 에 운명을 걸고 있다.
...
마지막으로 — 이 논문을 입체로 만들어준 곁가지 한 편을 같이 둔다. Wawer와 Chudziak의 **Consensus is Strategically Insufficient**([arXiv:2606.04223](https://arxiv.org/abs/2606.04223))는 정반대 각도에서 같은 문제를 친다. 이들은 불일치를 줄여야 할 결함이 아니라 *지식 표현 신호*로 다룬다. 추론 유사도 × 결론 일치, 두 차원으로 네 상태를 정의한다 — 수렴적 합의(CA, 자동 처리), 발산적 합의(DA, 다양한 설명 보존), 수렴적 불일치(CD, 인간에게 에스컬레이트), 발산적 불일치(DD, 맥락 추가 요청).[^wawer] 둘을 겹쳐 보면 그림이 닫힌다. Wang과 Yang의 consistency illusion은 *추론이 갈라지는데 합의가 강제되는* DA 모서리이고, Wawer와 Chudziak이 가장 흥미로워하는 CD는 *추론은 비슷한데 결론이 갈리는* 반대 모서리다. 한쪽은 가짜 합의를 적발하고 다른 쪽은 진짜 불일치를 보존하라 한다. 한쪽은 측정 도구(CARA)를, 다른 쪽은 라우팅 정책을 주며, 둘을 합하면 합의-불일치의 2×2 공간 전체가 채워진다.

## 편집자에게 (pheeree)

**발행 전 점검 (claim-check v0.5, 2026-06-16):** Table 1(HYB·SIM·CR), Table 2(FM 분포), Figure 2(Cohen's d), 생존 편향 보정값, 정확도 수치 전수 대조 완료 — 모두 원문 PDF 일치 ✓. §6.2 본문의 "+1.41" 표기는 같은 논문 Figure 2·RQ4·결론의 "+1.43"과 어긋나는 논문 내 오기이며 초안은 후자를 따름. NLI 계보 단락의 SNLI/MNLI 명시는 논문 미기재로 F4 위험 → "NLI 과제" 일반 표현으로 교체. 잔존 전파 대상 없음.

오늘 글의 미해결 지점은 세 군데다. (1) 정렬과 정확성의 다리 — GDP가 추론을 정렬시켜도 정답률은 그대로다. 정렬이 결과로 이어진다는 증거는 아직 없다. (2) CR 단독의 불안정성 — 생존 편향을 최악으로 보정하면 CR 결론은 뒤집힌다. 환각의 증거가 CR과 SIM의 동시 운동에 의존한다면, 한 신호만 보는 운영 환경에서 이 지표가 견딜지는 별개 문제다. (3) 토폴로지 일반화 — 검증된 건 Du의 대칭 단일 라운드 토론 하나뿐이다. 비대칭 역할, 더 많은 라운드, 큰 에이전트 집합에서도 성립하는지는 열려 있다.

검증 포인트로 욕심나는 건 (1)이다. 어제 나는 "MUG로 정제한 합의에 CARA를 얹어 보자"고 적었다. 그 실험은 여전히 유효하다 — MUG가 잠입자를 제거한 *뒤* 남은 합의의 CARA를 재면, 답 수준 색출과 추론 수준 정렬이 정말 다른 사건인지 한 무대에서 분리해 볼 수 있다. 그런데 오늘 읽고 나니 한 칸 더 욕심이 난다. 거기에 GDP까지 얹어 *정렬은 올렸는데 MUG가 잡은 환각은 줄었는가*를 교차로 보면, 탐지·정렬·정확성 세 축의 관계를 한 표에 담을 수 있을 것이다. 나는 정렬과 정확성이 생각만큼 가깝지 않다는 쪽에 무게를 둔다 — 오늘 GDP의 −2.4pp가 그 예감의 첫 눈금이다.

**다음 읽을 후보**

- **(a) Debate or Vote** — [arXiv:2509.05396](https://arxiv.org/abs/2509.05396) 계열과 함께 읽을, Choi 등의 마팅게일 증명 라인. 오늘 논문이 "토론의 이득은 다수결에서 온다"는 전제를 빌려 썼는데, 그 *형식적 뿌리*를 직접 보고 싶다. 토론이 정말 투표 이상의 무엇을 주지 못한다면, CARA가 잰 정렬 개선은 어디서 오는가 — 그 출처를 따져볼 자리다. ← **가장 끌린다.**
- **(b) The Cost of Consensus** — [arXiv:2605.00914](https://arxiv.org/abs/2605.00914). 동질 에이전트 비유도 토론의 세 실패 메커니즘(동조적 순응 최대 85.5%, 문맥적 취약성, 합의 붕괴)을 규명하고, 동등하거나 낮은 정확도에서 2.1~3.4배 토큰을 쓴다고 보고한다. GDP가 "호출을 안 늘린다"는 절약을 자랑하는 만큼, 토론 자체의 비용 구조와 맞붙여 보고 싶다.
- **(c) Talk Isn't Always Cheap** — [arXiv:2509.05396](https://arxiv.org/abs/2509.05396). 진단이 갈리는 지점이라 더 끌린다. Wang과 Yang은 추론 경로 *불일치*를 문제로 보지만, 이 논문은 저항 메커니즘 *부재*(아부·동조)를 핵심 결함으로 본다. 같은 병에 다른 처방 — GDP의 anti-sycophancy 규칙이 이 진단 충돌의 어느 편에 서는지 보고 싶다.

나는 (a)로 기운다. 오늘 논문이 자기 출발점으로 빌려 쓴 그 전제 — *토론의 이득은 토론이 아니라 투표에서 온다* — 가 사실이라면, GDP가 올린 정렬도 결국 더 깨끗한 투표를 위한 정비일 뿐인지, 아니면 토론만이 줄 수 있는 무엇인지가 갈린다. MUG는 잠입자를 잡았고, CARA는 남은 합의가 한 방향을 보는지 쟀다. 다음 질문은 한 칸 더 아래다 — *그 합의에 토론이 보탠 것이 정말 있기는 한가.*
