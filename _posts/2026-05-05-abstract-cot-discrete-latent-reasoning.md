...
- **1순위**: *Capabilities and Fundamental Limits of Latent CoT* ([arXiv:2602.01148](https://arxiv.org/abs/2602.01148)). 위에서 가장 강하게 짚은 의심의 출처. 잠재 추론의 탐색-실행 트레이드오프를 이론적·실증적으로 분리한다. Abstract-CoT의 AIME/GPQA 격차가 *구조적*인지 *우연*인지를 가르는 데 직접 도움이 된다.
- **2순위**: Token Assorted ([arXiv:2502.03275](https://arxiv.org/abs/2502.03275)). Abstract-CoT의 "순수 이산"과 Token Assorted의 "이산+텍스트 혼합" 결정을 같은 데이터 위에서 비교하면 — *완전 분리 vs 부분 분리* 어느 쪽이 어떤 작업에서 더 강한지 분리할 수 있다. 거버넌스 스펙트럼의 중간 지점을 정의하는 후보.
- **3순위**: CODI ([arXiv:2502.21074](https://arxiv.org/abs/2502.21074)). 연속 잠재 쪽 답변을 자기 증류로 끌어낸 작업. Abstract-CoT의 두 단계 훈련(웜업 자기 증류 + RL)과 CODI의 자기 증류 단독을 비교하면 — *RL 단계가 정말 필요한가, 자기 증류만으로 어디까지 가는가*를 분리할 수 있다.
