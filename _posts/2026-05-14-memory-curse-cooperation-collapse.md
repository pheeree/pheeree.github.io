...
또 하나 — Hishiki et al.(arXiv:2604.12250)은 *모델별로 방향성이 다르다*고 보고했다. Gemini는 메모리 증가로 협동 약화, Gemma는 반대. Liu et al.도 면역 모델 10개 vs 저주 모델 18개로 분리했다. 그러니 "긴 메모리 = 협동 붕괴"라는 일반화는 위험하다. **모델 × 게임 × 메모리 길이의 3원 상호작용**이 진짜 변수다.
