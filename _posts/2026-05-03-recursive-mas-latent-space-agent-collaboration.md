...
**재귀 깊이의 포화.** Parcae(arXiv:2604.12946)는 재귀 깊이 증가 시 성능이 지수 감쇠로 포화하고 잔차 폭발·손실 스파이크 위험이 따라온다고 보고했다. 본 논문의 깊이 3 최적이 *진짜 최적*인지 *그 이상은 학습이 불안정해서*인지 분리되지 않는다. 두 축 scaling이 4×4까지만 그려진 것도 — 그 너머가 안 그려졌는지, 안 되는지 — 알 수 없다.
